...
router.get("/:username", async(req, res) => {
    try {
        ...
        const posts = await Post.find({ use: user._id })
            .sort({ createdAt: -1 })
            .populate("user");
...
router.get("/:username/followers", async(req, res) => {
    try {
        ...
        const followers = await Followers.findOne({ user: user._id }).populate(
            "followers.user"
        );
...
router.put("/", auth, async(req, res) => {
    try {
        ...
        profile = await Profile.findOneAndUpdate({ user: req.userId }, { bio, techStack, social }, { new: true });
